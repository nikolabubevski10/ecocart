...
function registerGoalTaxonomy()
{
    $labels = [
        'name'                       => _x('Goals', 'Taxonomy General Name', 'flynt'),
        'singular_name'              => _x('Goal', 'Taxonomy Singular Name', 'flynt'),
        'menu_name'                  => __('Goal', 'flynt'),
    ...
    register_taxonomy('goal', ['offset-project'], $args);
}

add_action('init', 'Flynt\\CustomTaxonomies\\registerGoalTaxonomy');

function set_custom_edit_goal_columns($columns)
{
    return [
        'cb' => $columns['cb'],
        'image' => 'Image',
...
add_filter('manage_edit-goal_columns', '\\Flynt\\CustomTaxonomies\\set_custom_edit_goal_columns');

function custom_goal_column($string, $column, $term_id)
{
    if ($column == 'image') {
        echo '<img src="' . get_field('image', 'goal_' . $term_id) . '" style="width: 100px;"/>';
    }
}

add_action('manage_goal_custom_column', '\\Flynt\\CustomTaxonomies\\custom_goal_column', 10, 3);
